# Installing the Latest Brain Update → v1.5.7 (complete reliability release)

Hi — this brings your brain to the current release (**v1.5.7**): full **self-healing**
for both the brain itself AND its public tunnel, and it stops your access log from
recording API keys in plaintext. Everything here runs on your own machine and your
own accounts — nothing connects back to anyone else's setup.

> **This is the single complete install.** If you haven't installed v1.5.6 yet, you do
> NOT need to do it separately — v1.5.7 contains everything (self-heal watchdog with
> ghost-process clearing, safe restart tool, off-machine alarm, tunnel auto-fixer, and
> access-log key redaction). Install from v1.5.7 or later only.
...
- **Tunnel auto-fixer (v1.5.6):** your brain can be healthy at home while the
  PUBLIC side is dark — the Cloudflare tunnel's connections can wedge. A second silent
  helper fixes that: if the public side is down while the brain is fine locally, it
  restarts the tunnel automatically. It never texts (the off-machine alarm below stays
  the only loud voice).
- **Access-log key redaction (new in v1.5.7):** the HTTP access log records request
  URLs, and connector requests carry your API key in the URL — so the key was being
  written to the log in plaintext. v1.5.7 masks it before it's written. This changes
  ONLY what's logged: a valid key still works, an invalid key is still rejected — the
  log just stops showing the value.
...
[ "$(git rev-parse HEAD)" = "$(git rev-list -n1 v1.5.7)" ] && echo "on v1.5.7 ✓"
ls update/reliability/brain-watchdog.sh update/reliability/brain-restart.sh update/reliability/brain-tunnel-watchdog.sh update/reliability/log-redact/sitecustomize.py update/RUNBOOK-v1.4.md
```
...
code on disk is unchanged. Checking out v1.5.7 only adds the new reliability files to your
clone (which you set up in Sections 5–6). Nothing about your running brain changes at this step.

**CHECK:** prints `on v1.5.7 ✓` and the five files exist.
...
## 6c. Access-log key redaction (new in v1.5.7)

Your HTTP access log records request URLs, and connector requests carry the key
as `?api_key=...` — so the key was landing in the log in plaintext. v1.5.7 masks
it at the logging layer, **before it is written**, with zero change to how requests
are authenticated.

**Install the redaction shim** (engine source untouched):
```bash
mkdir -p ~/bin/log-redact
cp update/reliability/log-redact/sitecustomize.py ~/bin/log-redact/
```
Add ONE line to your `<INSTALL_DIR>/memory-server.sh`, right after the
`source .../activate` line:
```bash
export PYTHONPATH="$HOME/bin/log-redact${PYTHONPATH:+:$PYTHONPATH}"
```
Restart and confirm the log now redacts:
```bash
~/bin/brain-restart.sh restart
# fire one authed request, then:
grep -c 'api_key=REDACTED' <your access log>   # should be > 0
grep -c '<the real key>'   <your access log>   # should be 0
```
**Scrub the history + lock down perms** (the existing log already holds past keys):
```bash
python3 - "<your access log path>" <<'PY'
import re,sys
...
with open(p,'r+') as f:            # inode-preserving: the live server keeps appending
...
> handling — if the shim ever fails to load, the server still starts and still
> authenticates; you'd just lose redaction (the grep check above catches that).
